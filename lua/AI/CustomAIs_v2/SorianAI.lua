#****************************************************************************
#**
#**  File     :  /lua/AI/CustomAIs_v2/SorianAI.lua
#**  Author(s): Michael Robbins aka Sorian
#**
#**  Summary  : Utility File to insert custom AI into the game.
#**
#****************************************************************************

AI = {
	Name = "Sorian AI",
	Version = "2.1.1",
	AIList = {
		{
			key = 'sorian',
			name = "<LOC sorian_0001>AI: Sorian AI",
		},
		{
			key = 'sorianrush',
			name = "<LOC sorian_0003>AI: Sorian AI Rush",
		},
		{
			key = 'sorianair',
			name = "<LOC sorian_0005>AI: Sorian AI Air",
		},
		{
			key = 'sorianwater',
			name = "<LOC sorian_0007>AI: Sorian AI Water",
		},
		{
			key = 'sorianturtle',
			name = "<LOC sorian_0009>AI: Sorian AI Turtle",
		},
		{
			key = 'sorianadaptive',
			name = "<LOC sorian_0011>AI: Sorian AI Adaptive",
		},
	},
	CheatAIList = {
		{
			key = 'soriancheat',
			name = "<LOC sorian_0013>AIx: Sorian AI",
		},
		{
			key = 'sorianrushcheat',
			name = "<LOC sorian_0015>AIx: Sorian AI Rush",
		},
		{
			key = 'sorianaircheat',
			name = "<LOC sorian_0017>AIx: Sorian AI Air",
		},
		{
			key = 'sorianwatercheat',
			name = "<LOC sorian_0019>AIx: Sorian AI Water",
		},
		{
			key = 'sorianturtlecheat',
			name = "<LOC sorian_0021>AIx: Sorian AI Turtle",
		},
		{
			key = 'sorianadaptivecheat',
			name = "<LOC sorian_0023>AIx: Sorian AI Adaptive",
		},
	},
}