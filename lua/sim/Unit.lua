...
local nextRefresh
local AdpCheatMult = tonumber(ScenarioInfo.Options.CheatMult)
local AdpBuildMult = tonumber(ScenarioInfo.Options.BuildMult)
...
function SetupCheat(aiBrain,unit,BuildMult,CheatMult,HealthMult)
   
        aiBrain.CheatEnabled = true
        
        local buffDef = Buffs['CheatBuildRate']
        local buffAffects = buffDef.Affects
		if not (EntityCategoryContains(categories.NUKE, unit) or (EntityCategoryContains(categories.ANTIMISSILE, unit) and EntityCategoryContains(categories.SILO, unit))) then
			buffAffects.BuildRate.Mult = tonumber(BuildMult)
		else
			buffAffects.BuildRate.Mult = 1
		end
        #LOG('*AI DEBUG: Build Rate Modified to: '..tonumber(ScenarioInfo.Options.BuildMult))

        buffDef = Buffs['CheatIncome']
        buffAffects = buffDef.Affects
        buffAffects.EnergyProduction.Mult = tonumber(CheatMult)
        buffAffects.MassProduction.Mult = tonumber(CheatMult)
        #LOG('*AI DEBUG: Resource Rate Modified to: '..tonumber(ScenarioInfo.Options.CheatMult))
		
		---------设人类作弊血量
		unit:SetMaxHealth(unit:GetMaxHealth() * tonumber(HealthMult))
		if EntityCategoryContains(categories.COMMAND, unit) then
			unit:SetHealth(unit, unit:GetMaxHealth())
		end

     
            ApplyCheatBuffsS(unit)
    
end

function SetupAICheat(aiBrain,unit,BuildMult,CheatMult)
		...
		if ScenarioInfo.Options.AIMultHP ~= "off" then
            ...
            ApplyCheatBuffs(unit)
			
		----------
    
end

function SetupAICheatS(aiBrain,unit,BuildMult,CheatMult)
...
function ApplyCheatBuffs(unit)
	...
	if ScenarioInfo.Options.BuildMult == 'AUTOAIB3' or ScenarioInfo.Options.CheatMult == 'AUTOAIB2' or ScenarioInfo.Options.CheatMult == 'AUTOAIB1' then
        Buff.ApplyBuff(unit, 'IntelCheat')
        #LOG('*AI DEBUG: Omni Cheat is: '..ScenarioInfo.Options.OmniCheat)
    end
...
function ApplyCheatBuffsS(unit)
    Buff.ApplyBuff(unit, 'CheatIncome')
	
	Buff.ApplyBuff(unit, 'CheatBuildRate')
end

function RemoveCheatBuffs(unit)

    Buff.RemoveBuff(unit, 'CheatIncome')
	
	Buff.RemoveBuff(unit, 'CheatBuildRate')
...
PrintCake = function(UBBBPID)
		...
		PrintText(UBBBPID, 10, 'fff57900', 20, 'center')
	
end

local function LoadIcon(num, unit)
	if num == 2 then 
		unit:SetStrategicUnderlay('icon_objective_primary')
		--bp.StrategicIconName = 'icon_ACU_generic' --修改战略图标（无法刷新导致不能用）
	elseif num == 1 then
...
--FXF210302 自定义战略图标
local function CustomStrategicIcon(unit)
	if unit then
		--读取单位bp
	...
	--LOG('***here!***:'..repr(bp.StrategicIconName))
	local IconType --为了以后改进算法设置的
		--单位核查 
		-----------------------陆军--------------------------
		
		-----------------------------------------------------
		if unit and EntityCategoryContains(categories.LAND, unit) then
			--T4
			if EntityCategoryContains(categories.EXPERIMENTAL, unit) then
				--
					IconType = 'icon_T4LAND'
					if ScenarioInfo.Options[IconType] then 
						LoadIcon(ScenarioInfo.Options[IconType], unit)
					end
			--ACU
			...
			--狙击
			elseif EntityCategoryContains(categories.xal0305, unit) or EntityCategoryContains(categories.xsl0305, unit) then
				--
					IconType = 'icon_T3SNP'
					if ScenarioInfo.Options[IconType] then 
						LoadIcon(ScenarioInfo.Options[IconType], unit)
			...
			--自爆虫
			elseif EntityCategoryContains(categories.xrl0302, unit) then
				--
					IconType = 'icon_T2MBB'
					if ScenarioInfo.Options[IconType] then 
						LoadIcon(ScenarioInfo.Options[IconType], unit)
		...
		-----------------------------------------------------
		elseif unit and EntityCategoryContains(categories.AIR, unit) then
			--T4
			if EntityCategoryContains(categories.EXPERIMENTAL, unit) then
				--
					IconType = 'icon_T4LAND'
		...
		-----------------------------------------------------
		elseif unit and EntityCategoryContains(categories.NAVAL, unit) then
			--T4
			if EntityCategoryContains(categories.EXPERIMENTAL, unit) then
				--
					IconType = 'icon_T4NAVY'
					if ScenarioInfo.Options[IconType] then 
						LoadIcon(ScenarioInfo.Options[IconType], unit)
					end
			--巡洋
			...
			--T2潜艇
			elseif EntityCategoryContains(categories.T2SUBMARINE, unit) then
				--
					IconType = 'icon_T2SS'
					if ScenarioInfo.Options[IconType] then 
						LoadIcon(ScenarioInfo.Options[IconType], unit)
			...
			elseif EntityCategoryContains(categories.xss0304, unit) then
				--
					IconType = 'icon_T3HUSS'
					if ScenarioInfo.Options[IconType] then 
						LoadIcon(ScenarioInfo.Options[IconType], unit)
					end
			--T3核潜艇
			elseif EntityCategoryContains(categories.NUKESUB, unit) then
				--
					IconType = 'icon_T3NKSS'
					if ScenarioInfo.Options[IconType] then 
						LoadIcon(ScenarioInfo.Options[IconType], unit)
					end
			--T3战列舰
			elseif EntityCategoryContains(categories.BATTLESHIP, unit) then
				--
					IconType = 'icon_T3BB'
					if ScenarioInfo.Options[IconType] then 
						LoadIcon(ScenarioInfo.Options[IconType], unit)
		...
		elseif unit and EntityCategoryContains(categories.STRUCTURE, unit) then
			--T4
			if EntityCategoryContains(categories.EXPERIMENTAL, unit) then
				--
					IconType = 'icon_T4BASE'
					if ScenarioInfo.Options[IconType] then 
						LoadIcon(ScenarioInfo.Options[IconType], unit)
					end
			--核弹
			elseif EntityCategoryContains(categories.NUKE, unit) then
				--
					IconType = 'icon_T3NK'
					if ScenarioInfo.Options[IconType] then 
						LoadIcon(ScenarioInfo.Options[IconType], unit)
			...
			--重炮
			elseif EntityCategoryContains(categories.ARTILLERY, unit) and EntityCategoryContains(categories.TECH3, unit) then
				--
					IconType = 'icon_T3HA'
					if ScenarioInfo.Options[IconType] then 
						LoadIcon(ScenarioInfo.Options[IconType], unit)
			...
			--饭盒
			elseif EntityCategoryContains(categories.ANTIMISSILE, unit) and EntityCategoryContains(categories.TECH3, unit) then
				--
					IconType = 'icon_T3SMD'
					if ScenarioInfo.Options[IconType] then 
						LoadIcon(ScenarioInfo.Options[IconType], unit)
			...
			--运输
			elseif EntityCategoryContains(categories.DEFENSE, unit) and EntityCategoryContains(categories.DIRECTFIRE, unit) then
				--T1
				if	EntityCategoryContains(categories.TECH1, unit) then
					IconType = 'icon_T1PD'
					if ScenarioInfo.Options[IconType] then 
...
Unit = Class(moho.unit_methods) {
    ...
    OnCreate = function(self)
		...
		if ScenarioInfo.type ~= 'campaign' then
			local AIBrain = self:GetAIBrain()
					...
					if   string.lower(AIBrain.BrainType) == 'human'  then
						...
						HumanBuildMult = ScenarioInfo.Options.HumaixBuild[nickname]
						HumanCheatMult = ScenarioInfo.Options.HumaixCheat[nickname]
						HumanHealthMult = ScenarioInfo.Options.HumaixHealth[nickname]
						
						SetupCheat(AIBrain,self,HumanBuildMult,HumanCheatMult,HumanHealthMult)
						
						self.HealthM = HumanHealthMult
						--LOG("Unit HealthMult set to:"..repr(self.HealthM))
					else
						if self:GetAIBrain().CheatEnabled then
							if ScenarioInfo.Options.BuildMult == 'AUTOAIB1' and ScenarioInfo.Options.CheatMult == 'AUTOAIC1' then
								local timeSecs = GetGameTimeSeconds()
								if timeSecs <= 360 then 
								SetupAICheat(AIBrain,self,1.5,1.5)
								--WARN("检查中(函数已触发1"..tostring(timeSecs)..")")
								elseif timeSecs >= 361 and timeSecs <= 900 then 
								SetupAICheat(AIBrain,self,2.0,2.0)--6
								--WARN("检查中(函数已触发2"..tostring(timeSecs)..")")
								elseif timeSecs >= 901 and timeSecs <= 1200 then
								SetupAICheat(AIBrain,self,2.3,2.3)--15
								elseif timeSecs >= 1201 and timeSecs <= 1500 then
								SetupAICheat(AIBrain,self,2.5,2.5)--20
								elseif timeSecs >= 1501 and timeSecs <= 1800 then
								SetupAICheat(AIBrain,self,2.7,2.7)--25
								elseif timeSecs >= 1801 and timeSecs <= 2100 then
								SetupAICheat(AIBrain,self,3.0,3.0)--30
								elseif timeSecs >= 2101 and timeSecs <= 2400 then
								SetupAICheat(AIBrain,self,6.0,6.0)--35
								elseif timeSecs >= 2401 then
								SetupAICheat(AIBrain,self,20.0,20.0)-->40
								else
								SetupAICheat(AIBrain,self,1000.0,1000.0)
								--WARN("检查中(函数已触发3"..tostring(timeSecs)..")")								
								end
							elseif ScenarioInfo.Options.BuildMult == 'AUTOAIB2' and ScenarioInfo.Options.CheatMult == 'AUTOAIC2' then
								local timeSecs = GetGameTimeSeconds()
								if timeSecs <= 360 then 
								SetupAICheat(AIBrain,self,1.7,1.7)
								--WARN("检查中(函数已触发1"..tostring(timeSecs)..")")
								elseif timeSecs >= 361 and timeSecs <= 900 then 
								SetupAICheat(AIBrain,self,2.0,2.0)--6
								--WARN("检查中(函数已触发2"..tostring(timeSecs)..")")
								elseif timeSecs >= 901 and timeSecs <= 1200 then
								SetupAICheat(AIBrain,self,2.5,2.5)--15
								elseif timeSecs >= 1201 and timeSecs <= 1500 then
								SetupAICheat(AIBrain,self,3.0,3.0)--20
								elseif timeSecs >= 1501 and timeSecs <= 1800 then
								SetupAICheat(AIBrain,self,5.0,5.0)--25
								elseif timeSecs >= 1801 and timeSecs <= 2100 then
								SetupAICheat(AIBrain,self,6.0,6.0)--30
								elseif timeSecs >= 2101 and timeSecs <= 2400 then
								SetupAICheat(AIBrain,self,10.0,10.0)--35
								elseif timeSecs >= 2401 then
								SetupAICheat(AIBrain,self,20.0,20.0)-->40
								else
								SetupAICheat(AIBrain,self,1000.0,1000.0)
							...
							elseif ScenarioInfo.Options.BuildMult == 'AUTOAIB3' and ScenarioInfo.Options.CheatMult == 'AUTOAIC3' then
								local timeSecs = GetGameTimeSeconds()
								if timeSecs <= 360 then 
								SetupAICheat(AIBrain,self,2.0,2.0)
								--WARN("检查中(函数已触发1"..tostring(timeSecs)..")")
								elseif timeSecs >= 361 and timeSecs <= 900 then 
								SetupAICheat(AIBrain,self,2.5,2.5)--6
								--WARN("检查中(函数已触发2"..tostring(timeSecs)..")")
								elseif timeSecs >= 901 and timeSecs <= 1200 then
								SetupAICheat(AIBrain,self,3.0,3.0)--15
								elseif timeSecs >= 1201 and timeSecs <= 1500 then
								SetupAICheat(AIBrain,self,4.0,4.0)--20
								elseif timeSecs >= 1501 and timeSecs <= 1800 then
								SetupAICheat(AIBrain,self,6.0,6.0)--25
								elseif timeSecs >= 1801 and timeSecs <= 2100 then
								SetupAICheat(AIBrain,self,10.0,10.0)--30
								elseif timeSecs >= 2101 and timeSecs <= 2400 then
								SetupAICheat(AIBrain,self,50.0,50.0)--35
								elseif timeSecs >= 2401 then
								SetupAICheat(AIBrain,self,100.0,100.0)-->40
								else
								SetupAICheat(AIBrain,self,1000.0,1000.0)
								--WARN("检查中(函数已触发3"..tostring(timeSecs)..")")								
								end
								...
								AIBuildMult=tonumber(ScenarioInfo.Options.BuildMult)
								AICheatMult=tonumber(ScenarioInfo.Options.CheatMult)
								SetupAICheat(AIBrain,self,AIBuildMult,AICheatMult)
							end
						end
		...
		--FXF210225 修改游戏终结者造价(保险）
		if ScenarioInfo.Options.Terminator == '1.5' then
			if EntityCategoryContains(categories.ORGTERMINATOR, self) or EntityCategoryContains(categories.DOUBLECOST, self) or EntityCategoryContains(categories.ATYPETER, self) then
				self:Destroy()
			end
		elseif ScenarioInfo.Options.Terminator == '1.25' then
			if EntityCategoryContains(categories.ORGTERMINATOR, self) or EntityCategoryContains(categories.OPFCOST, self) or EntityCategoryContains(categories.DOUBLECOST, self)then
				self:Destroy()
			end
		elseif ScenarioInfo.Options.Terminator == '2.0' then
			if EntityCategoryContains(categories.ORGTERMINATOR, self) or EntityCategoryContains(categories.OPFCOST, self) or EntityCategoryContains(categories.ATYPETER, self) then
				self:Destroy()
			end
		else
			if EntityCategoryContains(categories.OPFCOST, self) or EntityCategoryContains(categories.DOUBLECOST, self) or EntityCategoryContains(categories.ATYPETER, self) then
				self:Destroy()
			end
		end
		
		--FXF210302 测试图标
		CustomStrategicIcon(self)
    ...
    OnKilled = function(self, instigator, type, overkillRatio)
        ...
        if EntityCategoryContains(categories.COMMAND, self) then
            #LOG('*An ACU is dead')
			if instigator then
				LOG('*Killer:'..repr(instigator:GetArmy())..'*Dead:'..repr(self:GetArmy())..'*Time:'..repr(GetGameTimeSeconds()))
			else	
				LOG('*Killer:'..repr(self:GetArmy())..'*Dead:'..repr(self:GetArmy())..'*Time:'..repr(GetGameTimeSeconds()))
			end
...
--------------
    ...
    OnStartBuild = function(self, unitBeingBuilt, order)
		...
		--[[
				...
				local AIBrain = self:GetAIBrain()
				if   string.lower(AIBrain.BrainType) ~= 'human' and self:GetAIBrain().CheatEnabled then
			if ScenarioInfo.Options.BuildMult == 'AUTOAIB3' and ScenarioInfo.Options.CheatMult == 'AUTOAIC3' then

								local timeSecs = GetGameTimeSeconds()
								--RemoveCheatBuffs(self)
								WARN("检查中(作弊BUFF试图移除"..tostring(timeSecs)..")")
								if timeSecs <= 360 then 
								SetupAICheat(AIBrain,self,2.0,2.0)
								WARN("检查中(重复验证1"..tostring(timeSecs)..")")
								elseif timeSecs >= 361 and timeSecs <= 900 then 
								SetupAICheat(AIBrain,self,2.5,2.5)--6
								WARN("检查中(重复验证2"..tostring(timeSecs)..")")
								elseif timeSecs >= 901 and timeSecs <= 1200 then
								SetupAICheat(AIBrain,self,3.0,3.0)--15
								elseif timeSecs >= 1201 and timeSecs <= 1500 then
								SetupAICheat(AIBrain,self,4.0,4.0)--20
								...
								SetupAICheat(AIBrain,self,1000.0,1000.0)
								WARN("检查中(重复验证3"..tostring(timeSecs)..")")								
								end
			end
			end
		end
    	...
    	if not self:IsDead() then
			...
			if ScenarioInfo.type ~= 'campaign' then
				...
				if ScenarioInfo.Options.AdpAI ~= 'off' then
					...
					--LOG("检查中(数据输出"..refreshTime..nextRefresh..AdpCheatMult..AdpBuildMult..")")
					if   string.lower(AIBrain.BrainType) ~= 'human' and self:GetAIBrain().CheatEnabled then
						local timeSecs = GetGameTimeSeconds()
						RemoveCheatBuffs(self)
						...
						elseif timeSecs >= nextRefresh then
							...
							SetupAICheatS(AIBrain,self,AdpCheatMult,AdpBuildMult)
							nextRefresh = nextRefresh + refreshTime
							--LOG("检查中(buff刷新"..tostring(timeSecs)..")")
						end
					end
				end
			end
	...
	--战役修复
	CampaignPlayTeleportChargeEffects  = function(self)
	        local army = self:GetArmy()
        local bp = self:GetBlueprint()
