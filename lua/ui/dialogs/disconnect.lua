		...
		for index, client in clients do
            ...
            if client.connected then
                ...
                if client.quiet < 5000 then--and armiesInfo[index].outOfGame == false then	-- IF client no lag and playing ...
					...
					if t > 500 then
						LOG('*Disconnectto:'..repr(client.name))
					end
					if ForceEject then																				-- IF client lag timeout (+30 minute), kick !
					...
					elseif armiesInfo[Your_index].outOfGame and canEject then				-- IF ME is Observer
						...
						slot.eject:Hide()
					elseif canEject then -- or armiesInfo[index].outOfGame then 													-- IF client Lag 
						--LOG('>>> CanEject')
						slot.eject:Enable()
					end
