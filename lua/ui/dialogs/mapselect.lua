...
function CreateDialog(selectBehavior, exitBehavior, over, singlePlayer, defaultScenarioName, curOptions, availableMods, OnModsChanged)
	...
	--map filter
	namefilterTitle = Bitmap(filterGroup)
    LayoutHelpers.Below(namefilterTitle, filters[table.getn(filters)], 10)
	LayoutHelpers.AtLeftIn(namefilterTitle, filters[table.getn(filters)], 0)
	
	local comboBg = Bitmap(namefilterTitle)
    comboBg.Depth:Set(namefilterTitle.Depth)
    comboBg.Width:Set(290)
    comboBg.Height:Set(24)
    LayoutHelpers.AtLeftTopIn(comboBg, namefilterTitle, -2, -4)
    comboBg:SetTexture(UIUtil.UIFile('/dialogs/mapselect03/options-panel-bar_bmp.dds'))

	namefilterName = UIUtil.CreateText(namefilterTitle, "地图名", 16, UIUtil.bodyFont)
	LayoutHelpers.AtLeftTopIn(namefilterName, namefilterTitle, 2, 0)

    filterName = Edit(namefilterTitle)
    filterName.Width:Set(180)
    filterName.Height:Set(20)
    filterName:SetFont(UIUtil.bodyFont, 16)
    filterName:SetForegroundColor(UIUtil.fontColor)
	filterName:SetBackgroundColor("66666666")
    filterName:ShowBackground(true)
    filterName:SetDropShadow(true)
	LayoutHelpers.AtLeftTopIn(filterName, namefilterTitle, 100, -2)
...
function RefreshOptions(skipRefresh, singlePlayer)
    ...
    if skipRefresh then
        ...
        OptionSource[3] = {title = "<LOC AIop_01>AI Options", options = import('/lua/ui/lobby/lobbyOptions.lua').AIOpts}
    end
    OptionSource[4] = {}
...
function SetDescription(scen)
    ...
    if EnhancedLobby.CheckMapHasMarkers(scen) then
        description:AddItem("AI触发（可获得更好的AI效果）:有")
    else
        description:AddItem("AI触发（可获得更好的AI效果）:无")
    end
