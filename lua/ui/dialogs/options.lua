...
function CreateDialog(over, exitBehavior)
    ...
    function SetNewPage(tabControl)
        ...
        currentTabBitmap = Bitmap(dialog, UIUtil.SkinnableFile('/scx_menu/optab_btn/tab_btn_selected.dds'))
    ...
    for index, key in optionsOrder do
        tabData = options[key]
        local curButton = UIUtil.CreateButtonStd(dialog, '/scx_menu/optab_btn/tab', tabData.title, 16, 0, 0, "UI_Tab_Click_01", "UI_Tab_Rollover_01")
        curButton.label:SetDropShadow(true)
        if prev then
            LayoutHelpers.RightOf(curButton, prev, -8)
        else
