...
local function UpdateDisplay()
	...
	--ouput dialog data to moho log
	
		LOG('*DIALOG:  Player:"'..info.name..'"')
		LOG('{')	
		LOG('	Army = ' ..index)
		LOG('	Faction = ' ..info.faction)
		LOG('	Color = ' ..info.color)
		LOG('	Dialog = ' ..repr(curInfo.scoreData.current[index]))
		LOG('}')	
    end
	local GID = GetCommandLineArgOrDefault("/gameid", "none")
