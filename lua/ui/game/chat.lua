...
--]]
local AddUnicodeCharToEditText = import('/lua/UTF.lua').AddUnicodeCharToEditText --N

local CHAT_INACTIVITY_TIMEOUT = 25  -- in seconds
...
function CreateChatEdit()
    ...
    group.edit.OnNonTextKeyPressed = function(self, charcode, event)
	        if AddUnicodeCharToEditText(self, charcode) then --n
            return
        end
        GUI.bg.curTime = 0
    ...
    group.edit.OnEnterPressed = function(self, text)
        ...
        if text == "" then
            ...
            msg.text = text
			msg.fromid = GetFocusArmy()
            if ChatTo() == 'allies' then
				--LOG('*MESSAGE:{ Gamesec:"'..GetGameTimeSeconds()..'"'..' To:'..repr(msg.to)..' Speaker:'..GetFocusArmy()..' Text:'..repr(msg.text)..'}')
...
function ReceiveChat(sender, msg)
	LOG('*MESSAGE:{ Gamesec:"'..GetGameTimeSeconds()..'"'..' To:'..repr(msg.to)..' Speaker:'..repr(msg.fromid)..' Text:'..repr(msg.text)..'}')
