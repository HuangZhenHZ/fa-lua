...
function AssistMex(command)
	ForkThread(PrintCake,'AssistMex(command) line 1')
    local units = EntityCategoryFilterDown(categories.ENGINEER, command.Units)
	ForkThread(PrintCake,'local units = EntityCategoryFilterDown(categories.ENGINEER, command.Units)')
    if not units[1] then return end
	ForkThread(PrintCake,'if not units[1] then return end')
	...
	ForkThread(PrintCake,'cap = false')
    ...
    if cap then
		ForkThread(PrintCake,'SimCallback')
...
function OnCommandIssued(command)
	...
	if command.CommandType == 'Guard' and command.Target.EntityId then
        ...
        if EntityCategoryContains(categories.STRUCTURE * categories.MASSEXTRACTION, command.Blueprint) then
           ForkThread(PrintCake,'AssistMex(command)')
		
           local options = Prefs.GetFromCurrentProfile('options')
		   ForkThread(PrintCake,options['assist_mex'])
...
PrintCake = function(UBBBPID)
		...
		--PrintText(UBBBPID, 30, 'fff57900', 10, 'center')
