...
function ActionHandler(actions)
    ...
    for _, action in actions do
        ...
        elseif action.Action == 'accept' then
				if SessionGetScenarioInfo().Options.TeamSpawn == 'balances_smurf' or SessionGetScenarioInfo().Options.TeamSpawn == 'nomal_smurf' then
					Announce(LOCF('Smurf accepts your alliance.'))
				else
				    Announce(LOCF('<LOC diplomacy_0005>%s accepts your alliance.', armies[action.From].nickname))
				end
        elseif action.Action == 'reject' then
				if SessionGetScenarioInfo().Options.TeamSpawn == 'balances_smurf' or SessionGetScenarioInfo().Options.TeamSpawn == 'nomal_smurf' then
				Announce(LOCF('Smurf rejects your alliance.'))
				else
        ...
        elseif action.Action == 'never' then
				if SessionGetScenarioInfo().Options.TeamSpawn == 'balances_smurf' or SessionGetScenarioInfo().Options.TeamSpawn == 'nomal_smurf' then
				Announce(LOCF('Smurf is now ignoring your requests.'))
				else
        ...
        elseif action.Action == 'break' then
				if SessionGetScenarioInfo().Options.TeamSpawn == 'balances_smurf' or SessionGetScenarioInfo().Options.TeamSpawn == 'nomal_smurf' then
				Announce(LOCF('Your alliance with Smurf has been broken.'))
				else
...
function CreateOfferDialogue(action)
    local armies = GetArmiesTable().armiesTable
	if SessionGetScenarioInfo().Options.TeamSpawn == 'balances_smurf' or SessionGetScenarioInfo().Options.TeamSpawn == 'nomal_smurf' then
	    return UIUtil.QuickDialog(GetFrame(0), LOCF("Smurf has offered you an alliance."),
...
function BuildPlayerLines()
    ...
    local function CreateEntry(data, isAlly)
		...
		if SessionGetScenarioInfo().Options.TeamSpawn == 'balances_smurf' or SessionGetScenarioInfo().Options.TeamSpawn == 'nomal_smurf' then
        entry.name = UIUtil.CreateText(entry, 'Smurf', 16, UIUtil.bodyFont)
		else
        ...
        if isAlly then
            if data.human and not data.outOfGame then
                ...
                entry.giveUnitBtn.OnClick = function(self, modifiers)
					if SessionGetScenarioInfo().Options.TeamSpawn == 'balances_smurf' or SessionGetScenarioInfo().Options.TeamSpawn == 'nomal_smurf' then
					IUtil.QuickDialog(GetFrame(0), LOCF("Give Selected Units to Smurf?"),
                        '<LOC _Yes>', function()
