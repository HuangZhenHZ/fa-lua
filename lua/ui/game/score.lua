...
--[[
    ...
    local function CreateMapNameLine(data, armyIndex)
		...
		local playerHumBud = sessionInfo.Options.HumaixBuild[playerName]
		local playerHumEcm = sessionInfo.Options.HumaixCheat[playerName]
		local playerHumHlm = sessionInfo.Options.HumaixHealth[playerName]
		local playerspawn = sessionInfo.Options.TeamSpawn
        if (playerRating) then
			--Èç¹ûÊÇÂí¼×Ä£Ê½
		    if playerspawn and table.find({'balances_smurf', 'nomal_smurf'}, playerspawn) then
				playerNameLine = 'smurf'
				line.name:SetText(playerNameLine)
			else
				if playerHumBud ~= 1 or playerHumEcm ~= 1 or playerHumHlm ~= 1 then
				playerNameLine = playerName..' ['..math.floor(playerRating+0.5)..'] '..'(E:'..string.sub(string.format("%1f",tostring(playerHumEcm)),1,3)..' B:'..string.sub(string.format("%1f",tostring(playerHumBud)),1,3)..' H:'..string.sub(string.format("%1f",tostring(playerHumHlm)),1,3)..')'
				else
...
function CreateTimeLog()
	while (true) do
		LOG(string.format("*GAMETIME:%s", GetGameTime()))
		WaitSeconds(10) 
	end
