...
local function InitLobbyComm(protocol, localPort, desiredPlayerName, localPlayerUID, natTraversalProvider)
    ...
    lobbyComm.Connecting = function(self)
		if HasCommandLineArg ("/online") then
			connectingDialog = UIUtil.ShowInfoDialog(controlGroup, Strings.Connecting, "<LOC _Cancel>", ExitApplication)
		else
...
function CreateLobby(protocol, localPort, desiredPlayerName, localPlayerUID, natTraversalProvider)
	...
	if HasCommandLineArg ("/online") then
		connectdialog = UIUtil.ShowInfoDialog(parent, Strings.TryingToConnect, Strings.AbortConnect, ExitApplication)
	else
