...
--  control behvaior
	...
	--FACN????????
	if HasCommandLineArg ("/host") and HasCommandLineArg ("/ladder") then
		...
		local gameName = "autohost"
		local port = tonumber(GetCommandLineArgOrDefault("/port", "16001"))
        local protocol = "UDP"
		local function StartLobby(scenarioFileName)
            local lobby = import('/lua/ui/lobby/facnLadderLobby.lua')
            lobby.CreateLobby(protocol, port, playerName, nil, nil, over, exitBehavior)
            Prefs.SetToCurrentProfile('last_game_name', gameName)
            lobby.HostGame(gameName, scenarioFileName, false)
        end
	...
	if HasCommandLineArg ("/host") and HasCommandLineArg ("/port") then
		local function GetCommandLineArgOrDefault(argname, default)
			local arg = GetCommandLineArg(argname, 1)
			if arg then
				return arg[1]
