...
local LadderLobby = import('/lua/ui/lobby/facnLadderLobby.lua')
...
local function ReadCommandline()
    ...
    return {
        Address = GetCommandLineArgOrDefault("/address", "10.8.0.1"),
        Port = GetCommandLineArgOrDefault("/port", 5000),
		Name = GetCommandLineArgOrDefault("/player", "nobody"),
    }
end
local comlineData = ReadCommandline()
...
function CreateUI(over, exitBehavior)
	...
	if HasCommandLineArg ("/ladder") then
		lobby = import('/lua/ui/lobby/facnLadderLobby.lua')
	end
    ...
    createButton.OnClick = function(self)
		...
		if HasCommandLineArg ("/player") then
			name = comlineData.Name
		end
    ...
    ipconnectBtn.OnClick = function(self, modifiers)
		...
		if HasCommandLineArg ("/address") and HasCommandLineArg ("/port") then
			ipaddress = comlineData.Address
			portstr = comlineData.Port
			port = tonumber(comlineData.Port)
		end
...
-- discovery behaviors
	...
	if HasCommandLineArg ("/host") then
		local name = nameEdit:GetText()
			if HasCommandLineArg ("/player") then
				name = comlineData.Name
			end
